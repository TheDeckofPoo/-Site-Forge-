# Transport Build POC report (2026-09-11T19:17:55.731620+00:00)

- Areas: **2**
- Conveyor nodes: **7**
- Merges: **1**
- Wires: **2**
- Devices: **6**
- Unbound conveyors: **0**
- Untagged devices: **0**
- Autogen merges_2to1 rows: **1**

## Issues
- Transport_1: 'Transport_1_C1' is not a P### conveyor tag

## Areas
### Transport_1
- Conveyors: 7, wires: 2
- Merge **Merge 2:1** lanes=2 AOI=`Merge_2to1` discharge=`Transport_1_C1`
  - in0: `P310`
  - in1: `P124`

### Transport_2
- Conveyors: 0, wires: 0

## Autogen merges_2to1 (PLC2 shape)
- `Transport_1_C1` area=`Transport_1` lanes=2 P310 + P124 → Transport_1_C1 hold=`runhold`

Fragment file: `transport_autogen_merges_20260911-191755.json`

## Next steps
1. Bind every conveyor node to a P### tag from the RUN.
1. Tag motors as M### / VFD### only; ENC* for encoders; PE*/EZPE* for eyes.
1. 2:1 merges → Autogen merges_2to1 (PLC2 runhold). Import fragment into PLC Autogen workbook.
1. Keep sealed Fast_Conv / Slow_* / Merge_2to1 AOIs; only wire call sites.
1. Full L5X emit from this graph is Phase 2b (IO map + transport focus).
