...
def _configio_load_counts(run_dir: Path, machine: str) -> dict[str, Any]:
    """Gate 4 â€” machine-specific Configio.asc.{machine} load / Desc / join counts."""
...
def build_io_channel_trace(
    ...
    # Gate 3 â€” single model confirmation
    sole_model = {
        "statement": (
...
def render_trace_md(trace: dict[str, Any]) -> str:
    ...
    lines = [
        f"# I/O Channel Trace â€” {trace.get('machine')}",
        "",
    ...
    lines.extend(
        [
            ...
            "",
            "## Gate 1 â€” Manual end-to-end examples",
            "",
        ]
    )
    ...
    if assigned:
        lines.append(
            ...
            f"bit `{assigned.get('fortna_bit')}` â†’ **{assigned.get('owner_state')}** "
    ...
    lines.append("")
    lines.append("### Formerly false-UNRESOLVED unused bit â†’ PROVEN_SPARE")
    lines.append("")
    if spare:
        lines.append(
            f"- `{spare.get('physical_address')}` word `{spare.get('fortna_word')}` "
            f"bit `{spare.get('fortna_bit')}` â†’ **{spare.get('owner_state')}** "
            f"source `{spare.get('owner_source')}` Desc `{spare.get('configio_desc')}`"
        )
    else:
        lines.append("_not found on this RUN_")

    lines.extend(["", "## Gate 5 â€” Module source-vs-UI audit", ""])
