#!/usr/bin/env python3
"""Diagnose PLC2 blind-discovery → workbook → L5X merge pipeline fidelity.
...
Usage:
  python tools/scripts/diagnose_plc2_merge_pipeline.py
  python tools/scripts/diagnose_plc2_merge_pipeline.py --out exports/plc2-merge-discovery/PIPELINE_DIAGNOSTIC.md
"""
...
def omit_reason(
    ...
    *,
    ...
    if workbook_merge_count == 0:
        return (
            "not_in_workbook (merges_2to1 empty — discovery not auto-applied; "
            "transport_graph only emits asMerge|mergeConfirmed with >=2 inbound wires)"
        )
    if not in_workbook:
        ds = _norm_tag(m.get("downstream"))
        return (
            f"not_in_workbook (no merges_2to1 row matching boss/downstream={ds or '—'}; "
...
def render_markdown(diag: dict[str, Any]) -> str:
    ...
    lines.append(f"- Discovery: `{diag.get('discovery_path') or '—'}`")
    counts = diag.get("discovery_counts") or {}
    if counts:
        lines.append(
            f"- Discovery counts: proven={counts.get('proven', '—')} "
            f"candidate={counts.get('candidate', '—')} "
            f"unresolved={counts.get('unresolved', '—')} "
            f"total={counts.get('total', '—')}"
        )
    lines.append(
        f"- Workbook: `{diag.get('workbook_path') or '—'}` "
        f"(merges_2to1={diag.get('workbook_merges_2to1_count', 0)})"
    )
    lines.append(
        f"- L5X: `{diag.get('l5x_path') or '—'}` "
    ...
    if not calls:
        ...
        for c in calls:
            lines.append(
                f"- `{c.get('merge_tag')}`: {c.get('lane_a')} / {c.get('lane_b')} → {c.get('discharge')}"
            )
    lines.append("")
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(description="Diagnose PLC2 merge discovery→workbook→L5X pipeline")
