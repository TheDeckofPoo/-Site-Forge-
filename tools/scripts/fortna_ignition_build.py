...
Exports (not a full .gwbk yet):
  ...
  - tags_seed.json    Ignition-style tag tree draft (UDT-friendly paths)
...
def build_plc_aligned_tags(equipment: list[dict], machine: str, eip: dict | None = None) -> dict:
    ...
    # EIP module status placeholders (CPxRIOn from autogen)
    eip_tags = []
    for i, ad in enumerate((eip or {}).get("adapters") or []):
        ...
        if not rack:
            ip = ad.get("ip") or ""
            rack = "CP5"
            if ip.endswith((".54", ".55", ".56")):
                rack = "CP6"
            elif ip.endswith((".57", ".58")):
                rack = "CP7"
        # Will be refined to CPxRIOn order in build_package if topology available
        eip_tags.append(
            _atomic(
                _safe_tag(ad.get("name") or f"AENT_{i}"),
                dtype="Boolean",
                value_source="memory",
                doc=f"EIP adapter {ad.get('ip') or ''} ({rack})",
                tooltip=ad.get("ip") or "",
            )
        )
...
Generated by FortnaPlus `fortna_ignition_build.py`.
...
| tags_plc_aligned.json | Preferred tag tree (zones / conv / PE) |
| tags_seed.json | Legacy kind-folder seed |
...
FortnaPlus will automate embedding later; the JSON is the full placement + binding list today.
...
Rebuild from FortnaPlus **Ignition Build** tab (or CLI). New SVG uses a larger canvas,
...
def build_package(run_dir: Path, out_dir: Path | None = None, *, layout_mode: str = "equipment") -> dict:
    ...
    (out / "tags_seed.json").write_text(json.dumps(tags, indent=2), encoding="utf-8")
    ...
    try:
        ...
        (out / "COPY_TO_IGNITION.txt").write_text(
            "\n".join([
                "FortnaPlus Ignition project (from Build layout)",
                "=" * 48,
                f"Machine: {machine}",
    ...
    manifest = {
        ...
        "files": {
            ...
            "tags_seed": str(out / "tags_seed.json"),
...
def write_poc_ignition_script(path: Path, symbols: dict, machine: str) -> None:
    ...
    lines = [
        f"# FortnaPlus Ignition POC — {machine}",
    ...
    *,
    ...
    try:
        ...
        perspective_pack = pack_perspective_project(
            pack_dir,
            project_name=f"FortnaPlus_POC_{stamp}",
            symbols=symbols,
    ...
    manifest = {
        ...
        "gwbk_status": (
            ...
            f"Ignition import: perspective-import/FortnaPlus_POC "
...
def main() -> int:
    ap = argparse.ArgumentParser(description="FortnaPlus Ignition Build — layout + tag seed")
